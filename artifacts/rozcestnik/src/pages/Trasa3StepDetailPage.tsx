...
export default function Trasa3StepDetailPage() {
  const { slug } = useParams<{ slug: string }>();
  const step = trasa3Steps.find((s) => s.slug === slug);
  ...
  if (!step) {
    return (
      <PageLayout title="Místo" backPath="/trasy/3">
  ...
  return (
    <PageLayout title={step.place} backPath="/trasy/3">
      <div style={{
        display: "flex",
